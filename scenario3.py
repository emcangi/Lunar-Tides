...
Scenario 3:
    ...
    Constant M2 amplitude and phase (user designated)

Output (In both cases, {} gets filled in with the time step):
    genM2_dt={}_b{}_sc3.txt         Original generated tidal data
    reconM2_dt={}_b{}_sc3.txt       Tidal data binned by lunar local time and
                                    longitude
...
ends = ['2016-01-15',                 # End date for data generation
        '2016-01-23',
        '2016-01-30']
f1 = 'genM2_{}_dt={}_b{}_sc3.txt'     # File to write generated tidal data
f2 = 'reconM2_{}_dt={}_b{}_sc3.txt'   # File to write reconstructed tidal data
f3 = 'origM2_{}_dt={}_b{}_sc3.txt'    # File to write original M2 data after binsz
f4 = 'scenario3_results.txt'          # Prints results of χ² test, etc
...
f.write('Solar amplitude: {} (maximum; actual tide varies '
        'sinusoidally)\n'.format(a[1]))
f.write('Solar phase: 1 (maximum; actual tide varies sinusoidally)\n')
...
for end, cyc in zip(ends, cycle):
    for dt in DTS:
        for binsz in BINS:
            ...
            dataM = generate_tides(start, end, amps=a, ampflag=af, dt=dt,
...
# t = 'Original and reconstructed M2, full lunar cycle, binsz={} min, sc3'.format(
#     int(60*binsz))
