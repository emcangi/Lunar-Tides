...
line0 = line0.format('TIME-GCM Analysis', tide_type, 'Half lunar cycle, '
                     ...
                     'Amplitude', 'Phase', 'Vert offset', 'Date', 'Lat')
...
temp[:, 0] = temp[:, 0] - 2456300.7917
temp[:, 1] = temp[:, 1] + 88.75

temp2 = np.copy(phase_data)
temp2[:, 0] = temp2[:, 0] - 2456300.7917
...
column_labels = np.around(column_labels - 2456293.5416666665, 2)
...
plt.title('M2 tidal amplitude (zonal wind speeds)', y=1.05)
...
plt.savefig('m2tidebylat_amp.png', facecolor='w', edgecolor='w',
...
plt.rc('xtick', labelsize=8)
plt.rc('ytick', labelsize=12)
plt.title('M2 tidal phases (zonal wind speeds)', y=1.05)

maxtide = round(np.max(phases))
mintide = np.min(phases)
cbarticks = np.linspace(mintide, maxtide, 10)
cbarticklabels = ['{} m/s'.format(i) for i in cbarticks]
cbar = fig.colorbar(heatmap_P, ticks=cbarticks)
cbar.ax.set_yticklabels(cbarticklabels)

plt.savefig('m2tidebylat_phase.png', facecolor='w', edgecolor='w',
            format='png', transparent=False, bbox_inches='tight')
